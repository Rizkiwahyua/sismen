    ...
    <div class="max-w-7xl mx-auto pb-12">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                ...
                <p class="text-xs text-slate-400 mt-0.5">Kelola unit kerja beserta total dokumen terhubung</p>
            </div>
        ...
        <div class="mb-8">
            <div class="bg-gradient-to-br from-indigo-600 via-indigo-750 to-violet-800 rounded-2xl p-5 shadow-lg shadow-indigo-600/15 flex items-center justify-between max-w-sm hover:shadow-xl hover:shadow-indigo-600/20 hover:-translate-y-0.5 transition duration-200 text-white">
                <div class="space-y-1">
                    <p class="text-[10px] font-bold text-indigo-100 uppercase tracking-wider">Total Unit Kerja Terdaftar</p>
                    <p class="text-3xl font-black">
                        {{ $departments->count() }} <span class="text-xs font-medium text-indigo-200">Unit</span>
                    </p>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 {{ $color['border'] }} shadow-sm hover:shadow-xl transition-all duration-300 p-6 flex flex-col justify-between group relative overflow-hidden hover:-translate-y-1">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center justify-between mb-5">
                            <!-- Building Icon Box -->
                            <div class="w-9 h-9 rounded-xl bg-gradient-to-br {{ $color['from'] }} {{ $color['to'] }} text-white flex items-center justify-center shadow-sm">
                            ...
                            <div class="flex items-center gap-1.5">
                                ...
                                <form action="{{ route('admin.department.destroy', $department->id) }}" method="POST"
                                    onsubmit="return confirm('Apakah Anda yakin ingin menghapus unit kerja ini?')" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                        ...
                        <div class="space-y-3.5 mb-6">
                            <div>
                                <span class="text-[9px] font-bold text-slate-450 uppercase tracking-wider block mb-1">Nama Unit Kerja</span>
                                <h3 class="text-slate-800 text-xs font-black leading-snug line-clamp-2 min-h-[36px]" title="{{ $department->name }}">
                                    {{ $department->name }}
                                </h3>
                            </div>
                            ...
                            <div class="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold bg-slate-50/50 border border-slate-100 px-3 py-1.5 rounded-xl w-fit shadow-inner">
                                ...
                                <span>Memiliki <strong class="text-slate-800 font-extrabold text-xs ml-0.5">{{ $department->documents_count }}</strong> dokumen</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5 mb-2">
                            <div class="flex justify-between items-center text-[9px] font-bold text-slate-400 uppercase tracking-wide">
                                <span>Rasio Kepemilikan Dokumen</span>
                                <span>{{ min($department->documents_count * 10, 100) }}%</span>
                            </div>
